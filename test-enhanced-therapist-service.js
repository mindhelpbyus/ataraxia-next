...
async function testProfileUpdate(therapistId) {
  ...
  const params = {
    FunctionName: 'ataraxia-therapist-service',
    Payload: JSON.stringify({
      httpMethod: 'PUT',
      path: `/api/therapist/${therapistId}`,
      body: JSON.stringify(updateData),
      requestContext: { requestId: 'test-update-profile' }
    })
  };
...
async function testSpecialtyUpdate(therapistId) {
  ...
  const params = {
    ...
    Payload: JSON.stringify({
      ...
      requestContext: { requestId: 'test-update-specialties' }
    })
  };
...
async function testInsuranceUpdate(therapistId) {
  ...
  const params = {
    ...
    Payload: JSON.stringify({
      ...
      requestContext: { requestId: 'test-update-insurance' }
    })
  };
...
async function testCapacityUpdate(therapistId) {
  ...
  const params = {
    ...
    Payload: JSON.stringify({
      ...
      requestContext: { requestId: 'test-update-capacity' }
    })
  };
...
async function testGetCapacity(therapistId) {
  const params = {
    ...
    Payload: JSON.stringify({
      httpMethod: 'GET',
      path: `/api/therapist/${therapistId}/capacity`,
      requestContext: { requestId: 'test-get-capacity' }
    })
  };
  ...
  console.log('   📈 Capacity info:', {
    new_clients_capacity: data.capacity.new_clients_capacity,
    available_slots: data.capacity.available_new_client_slots,
    utilization: data.capacity.capacity_utilization_percent + '%'
  });
}
  ...
  for (const testQuery of testSearchQueries) {
    ...
    const params = {
      ...
      Payload: JSON.stringify({
        ...
        requestContext: { requestId: `test-search-${testQuery.name.replace(/\s+/g, '-')}` }
      })
    };
...
async function testTherapistMatching() {
  ...
  // Create client profile with preferences
  await pool.query(`
    INSERT INTO clients (
      user_id, preferred_session_format, preferred_therapist_gender,
      therapy_goals, city, state
    ) VALUES ($1, $2, $3, $4, $5, $6)
  `, [
    clientId,
    'video',
  ...
  const params = {
    ...
    Payload: JSON.stringify({
      ...
      requestContext: { requestId: 'test-matching' }
    })
  };
...
async function testGetComprehensiveProfile(therapistId) {
  const params = {
    ...
    Payload: JSON.stringify({
      httpMethod: 'GET',
      path: `/api/therapist/${therapistId}`,
      requestContext: { requestId: 'test-get-comprehensive' }
    })
  };
