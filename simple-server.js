...
/**
 * Simple Express Server for Ataraxia-Next Auth Service
 * Minimal wrapper for local development
 */
...
app.get('/auth/mfa/status', async (req, res) => {
  try {
    const lambdaEvent = expressToLambdaEvent(req);
    const lambdaResponse = await authHandler(lambdaEvent);
    lambdaToExpressResponse(lambdaResponse, res);
...
app.post('/auth/mfa/setup-totp', async (req, res) => {
  try {
    const lambdaEvent = expressToLambdaEvent(req);
    const lambdaResponse = await authHandler(lambdaEvent);
    lambdaToExpressResponse(lambdaResponse, res);
...
app.get('/client/therapists', async (req, res) => {
  try {
    const lambdaEvent = expressToLambdaEvent(req);
    const lambdaResponse = await authHandler(lambdaEvent);
    lambdaToExpressResponse(lambdaResponse, res);
...
// Generic handler for all other auth and client routes
