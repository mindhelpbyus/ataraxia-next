...
async function handleUpdateTherapist(therapistId, event, requestId, logContext) {
    ...
    try {
        ...
        const { first_name, last_name, phone_number, clinical_specialties, highest_degree, years_of_experience, bio, profile_image_url } = body;
        // Update user table
      ...
      `, therapistParams);
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to update therapist profile', requestId);
    }
}
/**
...
async function handleAdvancedSearch(event, requestId, logContext) {
    ...
    try {
        const { search, city, state, specialty, limit = '20', offset = '0' } = event.queryStringParameters || {};
        ...
        const transformedTherapists = therapists.map((row) => {
            ...
            try {
                if (row.clinical_specialties && typeof row.clinical_specialties === 'object') {
                    specialties = Object.keys(row.clinical_specialties).filter(k => row.clinical_specialties[k] === true || row.clinical_specialties[k] === 'true');
                }
            }
            catch (e) {
            ...
            try {
                if (row.therapeutic_modalities && typeof row.therapeutic_modalities === 'object') {
                    modalities = Object.keys(row.therapeutic_modalities).filter(k => row.therapeutic_modalities[k] === true || row.therapeutic_modalities[k] === 'true');
                }
            }
            catch (e) {
                // Ignore parsing errors
            ...
            try {
                if (row.session_formats && typeof row.session_formats === 'object') {
                    sessionFormats = Object.keys(row.session_formats).filter(k => row.session_formats[k] === true || row.session_formats[k] === 'true');
                }
            }
            catch (e) {
                // Ignore parsing errors
...
async function handleUpdateInsurance(therapistId, event, requestId, logContext) {
    ...
    try {
        ...
        if (updates.length === 0) {
            return (0, response_1.validationErrorResponse)('No insurance data provided', requestId);
        }
        updates.push(`updated_at = NOW()`);
        params.push(therapistId);
        await (0, database_1.query)(`
    ...
    `, params);
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to update insurance settings', requestId);
    }
}
/**
...
async function handleGetCapacity(therapistId, requestId, logContext) {
    ...
    `, [therapistId]);
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to retrieve capacity information', requestId);
    }
}
/**
...
async function handleUpdateCapacity(therapistId, event, requestId, logContext) {
    ...
    try {
        const body = JSON.parse(event.body || '{}');
        const { new_clients_capacity, max_caseload_capacity, emergency_same_day_capacity, preferred_scheduling_density, client_intake_speed } = body;
        const updates = [];
        const params = [];
        let paramIndex = 1;
        ...
        if (updates.length === 0) {
            return (0, response_1.validationErrorResponse)('No capacity data provided', requestId);
        }
        updates.push(`updated_at = NOW()`);
        params.push(therapistId);
        await (0, database_1.query)(`
    ...
    `, params);
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to update capacity settings', requestId);
    }
}
/**
...
async function handleGetMatchingTherapists(clientId, event, requestId, logContext) {
    ...
    `, [clientId]);
      ...
      LIMIT $6
    `;
        const matchingTherapists = await (0, database_1.query)(sql, [
            clientId,
        ...
        const transformedMatches = matchingTherapists.map((row) => {
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to find matching therapists', requestId);
    }
}
/**
 * Generate human-readable match reasons
 */
function generateMatchReasons(therapist, client) {
    const reasons = [];
...
async function handleUpdateAvailability(therapistId, event, requestId, logContext) {
    ...
    try {
        const body = JSON.parse(event.body || '{}');
        const { weekly_schedule, session_durations, session_formats, timezone, new_clients_capacity, max_caseload_capacity, emergency_same_day_capacity, preferred_scheduling_density } = body;
        const updates = [];
        const params = [];
        let paramIndex = 1;
        if (weekly_schedule) {
        ...
        if (updates.length === 0) {
            return (0, response_1.validationErrorResponse)('No availability data provided', requestId);
        }
        updates.push(`updated_at = NOW()`);
        params.push(therapistId);
        await (0, database_1.query)(`
...
//# sourceMappingURL=handler-backup.js.map