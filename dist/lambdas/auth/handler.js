 ...
 */
async function handleGetTherapistStatus(authProviderId, requestId, logContext) {
    const monitor = new logger_1.PerformanceMonitor(logger, 'get_therapist_status', { ...logContext, authProviderId });
    try {
...
async function handleLogin(event, requestId, logContext) {
    ...
    try {
        ...
        const user = await (0, database_1.queryOne)('SELECT * FROM users WHERE auth_provider_id = $1 AND auth_provider_type = $2', [cognitoSub, 'cognito']);
        if (!user) {
            monitor.end(false);
            return (0, response_1.errorResponse)(404, 'User not found in database', requestId);
        }
        monitor.end(true);
        return (0, response_1.successResponse)({
            user: {
        ...
        if (error.name === 'UserNotConfirmedException') {
            return (0, response_1.errorResponse)(400, 'Please confirm your email address first', requestId);
        }
...
async function handleRegister(event, requestId, logContext) {
    ...
    try {
        ...
        if (!email || !password || !firstName || !lastName) {
            return (0, response_1.validationErrorResponse)('Email, password, firstName, and lastName are required', requestId);
        }
        ...
        const user = await (0, database_1.queryOne)(`INSERT INTO users (
       ...
       RETURNING id, email, first_name, last_name, role, account_status`, [
        ...
        if (error.name === 'InvalidPasswordException') {
            return (0, response_1.errorResponse)(400, 'Password does not meet requirements', requestId);
        }
...
async function handleTherapistRegistration(event, requestId, logContext) {
    ...
    try {
        ...
        if (!firstName || !lastName || !licenseNumber || !licenseState) {
            return (0, response_1.validationErrorResponse)('Missing required fields: firstName, lastName, licenseNumber, licenseState', requestId);
        }
        ...
        if (orgInviteCode) {
         ...
         WHERE auth_provider_id = $2`, [inviteResult.organization_id, authProviderId]);
            // Update invite usage
         ...
         SET current_uses = current_uses + 1,
             ...
             status = CASE 
                 ...
                 ELSE 'active'
             END
         WHERE id = $2`, [authProviderId, inviteResult.id]);
            monitor.end(true);
            return (0, response_1.successResponse)({
        ...
        // Get user ID
        const user = await (0, database_1.queryOne)('SELECT id FROM users WHERE auth_provider_id = $1', [authProviderId]);
        if (!user) {
       ...
       ) VALUES ($1, $2, $3, $4, $5, $6)`, [user.id, regResult.id, 'registration_submitted', 'registration_created', 'success', 'system']);
        monitor.end(true);
        return (0, response_1.successResponse)({
...
async function handleConfirmSignUp(event, requestId, logContext) {
    ...
    try {
        ...
        if (!email || !confirmationCode) {
            return (0, response_1.validationErrorResponse)('Email and confirmation code are required', requestId);
        }
        ...
        await (0, database_1.query)('UPDATE users SET is_verified = true, account_status = $1 WHERE email = $2 AND auth_provider_type = $3', ['active', email, 'cognito']);
        monitor.end(true);
        return (0, response_1.successResponse)({
        ...
        if (error.name === 'CodeMismatchException') {
            return (0, response_1.errorResponse)(400, 'Invalid confirmation code', requestId);
        }
        if (error.name === 'ExpiredCodeException') {
            return (0, response_1.errorResponse)(400, 'Confirmation code has expired', requestId);
        }
        return (0, response_1.errorResponse)(500, 'Email confirmation failed', requestId);
    }
}
/**
...
async function handleResendConfirmationCode(event, requestId, logContext) {
    const monitor = new logger_1.PerformanceMonitor(logger, 'resend_confirmation', logContext);
    try {
        const body = JSON.parse(event.body || '{}');
        const { email } = body;
        if (!email) {
            return (0, response_1.validationErrorResponse)('Email is required', requestId);
        }
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to resend confirmation code', requestId);
    }
}
/**
...
async function handleForgotPassword(event, requestId, logContext) {
    ...
    try {
        ...
        const forgotCommand = new client_cognito_identity_provider_1.ForgotPasswordCommand({
            ClientId: COGNITO_CONFIG.clientId,
            Username: email
        });
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to initiate password reset', requestId);
    }
}
/**
...
async function handleConfirmNewPassword(event, requestId, logContext) {
    const monitor = new logger_1.PerformanceMonitor(logger, 'confirm_new_password', logContext);
    try {
        const body = JSON.parse(event.body || '{}');
        const { email, confirmationCode, newPassword } = body;
        if (!email || !confirmationCode || !newPassword) {
            return (0, response_1.validationErrorResponse)('Email, confirmation code, and new password are required', requestId);
        }
        ...
        return (0, response_1.errorResponse)(500, 'Password reset failed', requestId);
    }
}
/**
 * Handle get current user
 */
async function handleGetCurrentUser(event, requestId, logContext) {
    const monitor = new logger_1.PerformanceMonitor(logger, 'get_current_user', logContext);
    try {
        const authResult = await (0, auth_1.verifyJWT)(event.headers.Authorization);
        if (!authResult.success) {
            return (0, response_1.errorResponse)(401, authResult.error || 'Unauthorized', requestId);
        }
        const user = await (0, database_1.queryOne)('SELECT * FROM users WHERE auth_provider_id = $1 AND auth_provider_type = $2', [authResult.user.sub, 'cognito']);
        if (!user) {
            return (0, response_1.errorResponse)(404, 'User not found', requestId);
        }
        monitor.end(true);
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Failed to get current user', requestId);
    }
}
/**
 * Handle logout
...
async function handleLogout(event, requestId, logContext) {
    const monitor = new logger_1.PerformanceMonitor(logger, 'logout', logContext);
    try {
        ...
        monitor.end(false);
        return (0, response_1.errorResponse)(500, 'Logout failed', requestId);
    }
}
/**
 * Handle send phone code (placeholder for SMS verification)
 */
async function handleSendPhoneCode(event, requestId, logContext) {
 ...
 */
async function handleVerifyPhoneCode(event, requestId, logContext) {
 ...
 */
async function handleGoogleAuth(event, requestId, logContext) {
