...
import { verifyJWT, extractUserFromToken } from '../../shared/auth';
  ...
  try {
    ...
    if (email) {
      const emailCheck = await Promise.all([
        query(
          'SELECT id FROM temp_therapist_registrations WHERE email = $1 AND registration_status != $2',
          [email, 'rejected']
        ),
    ...
    if (phoneNumber) {
      const phoneCheck = await Promise.all([
        query(
          'SELECT id FROM temp_therapist_registrations WHERE phone_number = $1 AND registration_status != $2',
          [phoneNumber, 'rejected']
        ),
        query('SELECT id FROM users WHERE phone_number = $1', [phoneNumber])
      ]);
...
async function handleTherapistRegistration(
  ...
  try {
    ...
    if (!authProviderId || !firstName || !lastName || !licenseNumber || !licenseState) {
      return validationErrorResponse('Missing required fields: authProviderId, firstName, lastName, licenseNumber, licenseState', requestId);
    }
    ...
    if (orgInviteCode) {
      ...
      const userResult = await queryOne(
        `INSERT INTO users (
         ...
         RETURNING id`,
        [authProviderId, authProviderType, email, phoneNumber, firstName, lastName,
    ...
    await query(
      ...
      [userResult.id, regResult.id, 'registration_submitted', 'registration_created', 'success', 'system']
    );
...
async function handleDocumentUpload(
  ...
  const monitor = new PerformanceMonitor(logger, 'document_upload', { ...logContext, registrationId });
  ...
  try {
    ...
    if (!validTypes.includes(documentType)) {
      return validationErrorResponse('Invalid document type', requestId);
    }
    ...
    await query(
      ...
      [registrationId, user.id, documentType, documentUrl, originalFilename, fileSize, mimeType]
    );
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to upload document', requestId);
  }
}
...
async function handleGetDocuments(
  ...
  const monitor = new PerformanceMonitor(logger, 'get_documents', { ...logContext, registrationId });
  
  try {
    const documents = await query(
      `SELECT document_type, document_url, original_filename, file_size, 
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to retrieve documents', requestId);
  }
}
 ...
 */
async function handleGetPendingVerifications(
  event: APIGatewayProxyEvent,
  user: any,
  requestId: string,
  logContext: any
): Promise<APIGatewayProxyResult> {
  const monitor = new PerformanceMonitor(logger, 'get_pending_verifications', logContext);
  ...
  try {
    ...
    const countResult = await queryOne(
      `SELECT COUNT(*) as total FROM temp_therapist_registrations 
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to get pending verifications', requestId);
  }
}
 ...
 */
async function handleActivateTherapistAccount(
  registrationId: string,
  user: any,
  requestId: string,
  logContext: any
): Promise<APIGatewayProxyResult> {
  const monitor = new PerformanceMonitor(logger, 'activate_therapist_account', { ...logContext, registrationId });
  ...
  try {
    ...
    try {
      ...
      const tempData = await queryOne(
        'SELECT * FROM temp_therapist_registrations WHERE id = $1',
        [registrationId]
      );
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to activate therapist account', requestId);
  }
}
...
async function handleRejectTherapist(
  ...
  const monitor = new PerformanceMonitor(logger, 'reject_therapist', { ...logContext, registrationId });
  ...
  try {
    ...
    await query(
      `UPDATE temp_therapist_registrations
       SET registration_status = 'rejected',
           ...
           updated_at = NOW()
       WHERE id = $1`,
      [registrationId, user.id, rejectionReason]
    );
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to reject therapist', requestId);
  }
}
 ...
 */
async function handleInitiateBackgroundCheck(
  registrationId: string,
  user: any,
  requestId: string,
  logContext: any
): Promise<APIGatewayProxyResult> {
  const monitor = new PerformanceMonitor(logger, 'initiate_background_check', { ...logContext, registrationId });
  ...
  try {
    ...
    await query(
      ...
      [registrationId, 'background_check', 'background_check_initiated', 'success', 'admin', user.id]
    );
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to initiate background check', requestId);
  }
}
 ...
 */
async function handleCreateOrganizationInvite(
  event: APIGatewayProxyEvent,
  user: any,
  requestId: string,
  logContext: any
): Promise<APIGatewayProxyResult> {
  const monitor = new PerformanceMonitor(logger, 'create_org_invite', logContext);
  ...
  try {
    ...
    const invite = await queryOne(
      `INSERT INTO organization_invites (
       ...
       RETURNING *`,
      [organizationId, inviteCode, email, maxUses, expiresAt, user.id]
    );
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to create organization invite', requestId);
  }
}
 ...
 */
async function handleGetOrganizationInvites(
  event: APIGatewayProxyEvent,
  user: any,
  requestId: string,
  logContext: any
): Promise<APIGatewayProxyResult> {
  const monitor = new PerformanceMonitor(logger, 'get_org_invites', logContext);
  ...
  try {
    ...
    const invites = await query(sql, params);
    ...
    monitor.end(false);
    return errorResponse(500, 'Failed to get organization invites', requestId);
  }
}